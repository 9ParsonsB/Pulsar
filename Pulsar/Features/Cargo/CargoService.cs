...
public class CargoService(IOptions<PulsarConfiguration> options, ILogger<CargoService> logger, IEventHubContext hub)
    : ICargoService
{
    ...
    public async Task<CargoFile> Get()
    {
        ...
        if (!FileHelper.ValidateFile(cargoFile)) return new CargoFile();
