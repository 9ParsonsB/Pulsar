...
public class JournalEventArgs(string journalEventType, JsonObject journalEvent) : EventArgs
{
    /// <summary>
    ///     <para>
    ///         Elite Dangerous journal event, deserialized into a .NET object of the type specified by
    ///         JournalEventArgs.journalType.
    ///     </para>
    ///     <para>
    ///         Unhandled json values within a journal entry type will be contained in member property:<br />Dictionary&lt;
...
{
    /// <summary>
    ///     A value which allows grouping of notifications together. For example, values &gt;= 0 &lt;= 1000 could be system
    ///     body IDs, -1 is the system, anything else is arbitrary.
    /// </summary>
    ...
    ///     This value is optional, if omitted the value of <c>NotificationArgs.Detail</c> will be used for voice synthesis
    ///     without markup.
    /// </summary>
    public string DetailSsml;

    /// <summary>
    ///     Additional notification detailed (generally not rendered by voice or popup; potentially used by aggregating/logging
    ///     plugins).
    /// </summary>
    ...
    ///     <see cref="NotificationSuppression.None" />.
    /// </summary>
    public NotificationSuppression Suppression = NotificationSuppression.None;

    /// <summary>
    ///     Specify window timeout in ms (overrides Core setting). Specify 0 timeout to persist until removed via
    ///     IObservatoryCore.CancelNotification. Default -1 (use Core setting).
    /// </summary>
    ...
    ///     This value is optional, if omitted the value of <c>NotificationArgs.Title</c> will be used for voice synthesis
    ///     without markup.
    /// </summary>
    public string TitleSsml;

    /// <summary>
    ///     Specify window X position as a percentage from upper left corner (overrides Core setting). Default -1.0 (use Core
    ///     setting).
    /// </summary>
    public double XPos = -1.0;

    /// <summary>
    ///     Specify window Y position as a percentage from upper left corner (overrides Core setting). Default -1.0 (use Core
    ///     setting).
    /// </summary>
